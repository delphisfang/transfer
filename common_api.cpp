...
int CvtCharSet (const string &from_code, const string to_code, const string &src_str, string &dst_str)
{
	...
	if (m_cd == iconv_t(-1))
	{
		//DEBUG_P (LOG_ERROR, "[CvtGbkToU8] Create convert fd failed!\n");
		return -1;
	}
	...
	for (;;)
	{
		...
		else
		{
			...
			{
				case EILSEQ:
					//DEBUG_P (LOG_ERROR, "[CvtU8ToGbk] Invalid charset, errno:[%d], msg:[%s]!\n", errno, strerror(errno));
					++in_ptr;
					--in_left;
					*out_ptr = '\0';
					dst_str += string(out_buf);
					...
					//DEBUG_P (LOG_ERROR, "[CvtU8ToGbk] in_ptr:[%d], in_left:[%d]\n", in_ptr, in_left);
...
int getappID(const string& key)
{
	int pos = key.find('_');
	string strappID = key.substr(0, pos);
	return atoi(strappID.c_str());
}
